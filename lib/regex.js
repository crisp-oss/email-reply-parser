...
class RegexList {
  constructor() {
    this.quoteHeadersRegex = this.buildRe2([
      ...
      // Varations on the following, in various languages: 
      // "From: NAME <EMAIL>" OR "From : NAME <EMAIL>" OR "From : NAME<EMAIL>"(With support whitespace before start and before <)
      ...['From', 'Fra', 'Von', 'De', 'Van', 'Da'].map(
        fromString => new RegExp(`^\\s*(${fromString}\\s?:.+\\s?\\n?\\s*[\[|<].+[\\]|>])`, 'm')
      ),
    ...
    this.signatureRegex = this.buildRe2([
      ...
      new RegExp(`^-{1,10} ?(?:${["Original message", "Oprindelig besked", "Message d'origine"].join('|')}) ?-{1,10}$`, 'mi'), // Dash separators in multiple languages, they usually appear just above quote headers
      // DA
      /^Sendt fra (?:\s*.+)$/, // da
      // DE
      /^Von (?:\s*.+) gesendet$/, // de
      ...
      /^Yahoo Mail : (?:\s*.+)$/, // in fr, this can have "Recherchez, organisez, maîtrisez" trailing
      /^Cheers,?!?$/mi, // en
      /^Best wishes,?!?$/mi, // en
      ...
      // FR
      /^Envoyé depuis (?:\s*.+)$/, // fr
      /^Envoyé de (?:\s*.+)$/, // fr - e.g. Envoyé de mon iPhone
      /^Envoyé à partir de (?:\s*.+)$/, // fr - e.g. Envoyé à partir de Outlook pour Android
